...
import type { SettingsContextType, MainSettings } from './types/settings';
import {
	defaultSettings,
	getInitialSettings,
...
export const SettingsProvider = ({
	...
	const toggleEntity = useCallback((rowId: UniqueIdentifier) => {
		...
		setEntities((previousEntities: SortableEntity[]) => {
			const entityIndex = previousEntities.findIndex(
				({ id }) => id === rowId
			);
			const updated = [...previousEntities];
			updated[entityIndex].enabled =
				!previousEntities[entityIndex].enabled;
			return updated;
		});
	}, []);
	...
	const save = useCallback(() => {
		const data: {
			woo_buddy_main_settings: { enabled: string; provider: string };
